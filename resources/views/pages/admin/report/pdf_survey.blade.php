...
<body>
    ...
    <table class="table table-bordered">
        <tr>
            ...
            <th style="width: 15%;">RESPONDENT NAME</th>
            <th style="width: 10%;">RESPONSE DATE</th>
            <th style="width: 15%;">LINK SURVEY</th>
            <th style="width: 10%;">START DATE SURVEY</th>
            <th style="width: 10%;">END DATE SURVEY</th>
        </tr>
        @php
            $noreport = 1;
        @endphp
        @foreach ($reports as $data_report)
            <tr>
                ...
                <td>{{ $data_report->survey->link_survey }}</td>
                <td>{{ \Carbon\Carbon::parse($data_report->survey->start_date)->format('d-m-Y') }}</td>
                <td>{{ \Carbon\Carbon::parse($data_report->survey->end_date)->format('d-m-Y') }}</td>
            </tr>
        @endforeach
    </table>
